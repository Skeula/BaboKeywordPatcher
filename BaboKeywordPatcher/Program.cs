...
#pragma warning disable CS8604 // Possible null reference argument.
        public static void ParseName(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, IArmorGetter armor, String label, String name, String editorid)
        {
               ...
               )
            {
                ...
                {
                    matched = AddTags(armo, [ SLA_PantiesNormal, ClothingPanties ]) || matched;
                }
            ...
            {
                matched = AddTags(armo, [ClothingStrapOn, SexLabNoStrip]) || matched;
            }
            ...
            if (HasAnyTags(armo, [SLA_PantiesNormal, ClothingPanties]))
            {
                matched = AddTags(armo, [SLA_PantiesNormal, ClothingPanties, SLA_ArmorPartBottom]) || matched;
            }
            if (HasTag(armo, SLA_ThongT))
            {
                matched = AddTags(armo, [SLA_ArmorPartBottom]) || matched;
            }
            if (HasTag(armo, SLA_PantsNormal))
            {
                matched = AddTags(armo, [SLA_ArmorPartBottom]) || matched;
            }
            if (HasTag(armo, SLA_MicroHotpants))
            {
                matched = AddTags(armo, [SLA_ArmorPartBottom]) || matched;
            }

            //  Basic Synonyms
            if (HasTag(armo, SexLabNoStrip))
            {
                matched = AddTags(armo, [LE_NoStrip]) || matched;
            }
            if (HasTag(armo, LE_NoStrip)) {
                matched = AddTags(armo, [SexLabNoStrip]) || matched;
            }
